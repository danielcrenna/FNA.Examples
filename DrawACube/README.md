...
- Set up your game as per [Hello, World](https://github.com/danielcrenna/FNA.Examples/blob/main/HelloWorld/README.md)
- Rename the `HelloWorld.cs` class and file to `DrawACube.cs`
- In the class constructor, add code to create an array of vertices used to draw the cube:
...
- Use FNA.Package's `Input` helper class and override the game's `Update` method to handle keyboard commands to rotate the cube:
...
- Update the game's `Draw` command to draw the cube to the screen after the `GraphicsDevice.Clear` command:
